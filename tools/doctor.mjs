...
phase('Design tools')
{
  const tools = ['cgc.mjs', 'print-render.mjs', 'print-lint.mjs', 'screen-render.mjs', 'motion-render.mjs', 'techniques.mjs', 'check.mjs', 'icon-lint.mjs', 'slop-lint.mjs', 'page-audit.mjs', 'specimen.mjs', 'outline-text.mjs']
  ...
  const cgc = resolveExe('cgc')
  if (cgc) ok(`cgc on PATH (${Array.isArray(cgc) ? cgc[0] : cgc}) — every command the skills name runs in any project`)
