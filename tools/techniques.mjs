...
export const MEDIA = [
  {
    ...
    techniques: [
      ...
      T('grid-overlap', 'structure', 3, /grid-(?:area|column|row)\s*:\s*1\s*\/\s*1\b|grid-(row|column)\s*:\s*(\d+)\s*[;}][\s\S]{0,400}?grid-\1\s*:\s*\2\s*[;}]|z-index[\s\S]{0,80}margin-(?:top|left|bottom|right)\s*:\s*-/i, 'deliberate overlap — two elements sharing one grid cell, or a negative margin with z-index. The fastest way out of a column of rectangles: let something break its box.'),
