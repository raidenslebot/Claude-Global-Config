...
export const MEDIA = [
  {
    ...
    techniques: [
      ...
      T('houdini-paint', 'generative', 2, /CSS\.paintWorklet|paint\(\s*[a-z-]+\s*\)/i, 'a paint worklet — a background drawn by your own code, parameterised by custom properties. Chromium only: Firefox does not implement it and Safari ships it disabled, so it is an enhancement over a real background, never the background.'),
  ...
  },
  {
    ...
    techniques: [
      ...
      T('shader-effect', 'material', 3, /colorEffect|distortionEffect|layerEffect|ShaderMask|RenderEffect|AGSL|SkSL/i, 'a shader effect on a live view — distortion, grain, gradient masking, applied to real UI.'),
      ...
      T('haptics', 'response', 2, /UIImpactFeedback|sensoryFeedback|HapticFeedback|Vibration/i, 'haptics — a channel the web barely has. navigator.vibrate exists on Android, but a taptic engine with weight, sharpness and a real vocabulary of impacts does not.'),
  ...
  },
  {
    ...
    techniques: [
      ...
      T('overprint', 'material', 2, /overprint|knockout|trap(?:ping)?/i, 'overprint — one ink printed over another instead of knocking it out, so the two interact. That is a look screens have to fake. (Trapping, which the same word-family covers, is not an effect: it is a prepress correction for misregistration, invisible when it works and normally applied by the RIP rather than by you.)'),
