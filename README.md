...
**repairs** any failure by re-applying the install, starts this package's own tests once per
commit — in the background (`tools/selftest.mjs`), at reduced priority, so no session start
waits on them — and prints one line to the user and to the session, carrying the last finished
test result:
